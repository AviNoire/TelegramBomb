...
ask = input(f"{Fore.GREEN}TOKEN {Fore.RED}# ")
ask2 = input(f"{Fore.GREEN}id {Fore.RED}# ")
...
def start(message):
    ...
    buttons_to_add = [boom, stop, info, stats, piar, faq]

    if int(message.chat.id) == ADMIN_CHAT_ID:
        buttons_to_add.append(types.KeyboardButton(text='🔥Рассылка'))
        buttons_to_add.append(types.KeyboardButton(text='Заблокировать'))
        buttons_to_add.append(types.KeyboardButton(text='Разблокировать'))

    keyboard.add(*buttons_to_add)
    bot.send_message(message.chat.id, 'Добро пожаловать🙋‍♂!\nНаш канал: @codingbots \nВыберите действие:',  reply_markup=keyboard)
...
def handle_message_received(message):
    ...
    if text == 'ℹ️Информация':
        bot.send_message(chat_id, 'Создатель бота: @mossadx\nSpamm3r - 1.1\n\n<b>По вопросам сотрудничества обращаться в ЛС к создателю бота\n\n Наш канал: @codingbots</b>', parse_mode='HTML')
